...
# takes the path of a fasta file (filled with peptides which you want to find possible origin locations of)
# and returns a list of all these peptides.
...
# called to initiate the process of returning lin/cis/trans peptide locations. Takes the protein dict generated by
# protFastaToDict() and the pepList generate by pepFastaToList() as inputs. Also takes linFlag, cisFlag and transFlag
# run the code for the respective splice type if True.
def generateOrigins(protDict, pepList, outputPath, linFlag, cisFlag, transFlag):
    if linFlag:
        #lin
        linOriginDict = findLinOrigins(protDict, pepList)
        writeToFasta(linOriginDict, outputPath, 'Linear')
    if cisFlag:
        #cis
...
# takes protDict and pepList as the input, and outputs linOriginsDict. linOriginsDict has the peptide as a key, and
# a list of tuples of the form (originProtName, locations). Locations has different forms depending on the splice type, but
# generally stores information on where the corresponding peptide could have been generated from in the relevant
# origin prot.
...
def findLinOrigins(protDict, pepList):
    ...
    for pep in pepList:
        ...
        for protSeq in protDict.keys():
            ...
            locations = []
            # re.finditer(substring, string) returns an iterable with the start and finish indeces of all the locations
            # of the substring in the string. The iterator is empty if the susbet does not appear at all.
...
def generateOutput(outputPath, proteinFile, peptideFile, linFlag, cisFlag, transFlag):
    ...
    generateOrigins(protDict, pepList, outputPath, linFlag, cisFlag, transFlag)
